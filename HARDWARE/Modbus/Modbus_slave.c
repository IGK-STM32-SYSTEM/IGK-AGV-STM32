...
void AnalyzeRecieve()                              //分析并生成响应报文
{
	uint16_t _crc;
	uint8_t a1, a2;
	_crc = CalcCrc(rece3_buf, (rece3_index - 2)); //计算校验CRC
	a1 = _crc & 0xff;                      //CRC低字节
...
void Recirve_01()
{
	...
	else
	{
		...
		for(i = 0; i < send3_buf[2]; i++)
		{
			...
			{
				send3_buf[3 + i] = (uint8_t)((PLC_OutPut[startadd + i * 8 + j] & 0x01) << j) + send3_buf[3 + i]; //低位在前，高位在后

			}
		}
		aakj = CalcCrc(send3_buf, (send3_buf[2] + 3));            //CRC校验
...
void Recirve_02()// 搞定****************************************************
{
	...
	else
	{
		...
		for(i = 0; i < send3_buf[2]; i++)
		{
			...
			for(j = 0; j < 8; j++)            //每8个位状态组成一个字节返回
			{
				send3_buf[3 + i] = (uint8_t)((PLC_InPut[startadd + i * 8 + j] & 0x01) << j) + send3_buf[3 + i]; //低位在前，高位在后
...
void Recirve_03()
{
	...
	else
	{
		...
		for(i = 0; i < length; i++)
		{
			...
			//
			send3_buf[3 + i * 2] = (PLC_Data[startadd + i] >> 8) & 0xff; //返回寄存器值的高字节
			send3_buf[4 + i * 2] = PLC_Data[startadd + i] & 0xff; //返回寄存器值得低字节
		}
		aakj = CalcCrc(send3_buf, (length * 2) + 3);          //CRC校验
...
void Recirve_05()
{
	...
	else
	{
		...
		aakj = CalcCrc(send3_buf, 6);              //CRC校验
...
void Recirve_06()//单字节写入
{
	...
	else
	{
		...
		send3_buf[4] = (uint8_t)(((PLC_Data[startadd]) >> 8) & 0xff); //返回寄存器值高字节
		send3_buf[5] = (uint8_t)(PLC_Data[startadd] & 0xff);     //返回寄存器值低字节
		aakj = CalcCrc(send3_buf, 6);              //CRC校验
		send3_buf[6] = (uint8_t)(aakj & 0xff);               //CRC低字节
		send3_buf[7] = (uint8_t)((aakj >> 8) & 0xff);        //CRC高字节
